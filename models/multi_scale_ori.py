...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=10):
        ...
        self.maxpool3 = nn.AvgPool1d(kernel_size=10, stride=1, padding=0)  # 从16改为10
        ...
        self.maxpool5 = nn.AvgPool1d(kernel_size=6, stride=1, padding=0)  # 从11改为6
        ...
        self.fc = nn.Linear(128*3, 1)  # 从256改为128，从num_classes改为1
    ...
    def forward(self, x0):
        x0 = self.conv1(x0)
        x0 = self.bn1(x0)
        x0 = self.relu(x0)  # 64*79
        x0 = self.maxpool(x0)  # 64*40

        x = self.layer3x3_1(x0)  # 64*20
        x = self.layer3x3_2(x)  # 128*10
        # x = self.layer3x3_3(x)  # 256*5
        # x = self.layer3x3_4(x)
        x = self.maxpool3(x)  # 128*1

        y = self.layer5x5_1(x0)  # 64*17
        y = self.layer5x5_2(y)  # 128*6
        # y = self.layer5x5_3(y)
        # y = self.layer5x5_4(y)
        y = self.maxpool5(y)  # 128*1
