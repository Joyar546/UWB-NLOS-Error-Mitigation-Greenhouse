...
class SEResNet(nn.Module):
    ...
    def __init__(self, input_channel, layers, k_size=[3, 3, 3, 3]):
        ...
        self.layer1 = self._make_layer(SEBasicBlock, 64, layers[0], int(k_size[0]), stride=2)
        self.layer2 = self._make_layer(SEBasicBlock, 128, layers[1], int(k_size[1]), stride=2)
        self.layer3 = self._make_layer(SEBasicBlock, 256, layers[2], int(k_size[2]), stride=2)
        self.layer4 = self._make_layer(SEBasicBlock, 512, layers[3], int(k_size[3]), stride=2)
        self.avgpool = nn.AvgPool1d(kernel_size=4, stride=1)  # origin: 5, 4 for 128
        # self.fc1 = nn.Linear(512, 256)  # 8704 for FP1016, 512 for FP512
